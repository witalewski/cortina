import type { ChordLessonScore } from '@/app/types/chords';

interface ChordLessonSummaryProps {
  score: ChordLessonScore;
  onGoBack: () => void;
}

export function ChordLessonSummary({ score, onGoBack }: ChordLessonSummaryProps) {
  const percentage = Math.round((score.correctCount / score.totalChallenges) * 100);

  // Find the chord with the most attempts (struggling chord)
  const strugglingChord = score.results
        ...
        <div className="mb-6 rounded-lg border-2 border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-900/50 dark:bg-yellow-950/30">
          <p className="text-sm text-zinc-900 dark:text-zinc-50">
            ...
            <span className="font-semibold">
              {strugglingChord.challenge.displayName}
            </span>
            .
          </p>
        </div>
